...
resource "aws_iam_policy" "be_pipeline_role_policy" {
  name        = "be_pipeline_role_policy"
...
resource "aws_iam_policy_attachment" "be_pipeline_role_policy_attachment" {
  name       = "be_pipeline_role_policy_attachment"
  roles      = [aws_iam_role.be_pipeline_role.name]
  policy_arn = aws_iam_policy.be_pipeline_role_policy.arn
}
...
resource "aws_iam_policy_attachment" "be_codebuild_role_policy_attachment" {
  name       = "be_codebuild_role_policy_attachment"
  roles      = [aws_iam_role.be_codebuild_role.name]
  policy_arn = aws_iam_policy.be_codebuild_role_policy.arn
}

resource "aws_codebuild_project" "be_codebuild_tf_build" {
  name           = "be_codebuild_tf_build"
...
resource "aws_codebuild_project" "be_codebuild_tf_deploy" {
  name           = "be_codebuild_tf_deploy"
...
resource "aws_codepipeline" "be_codepipeline" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = aws_codebuild_project.be_codebuild_tf_build.id
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = aws_codebuild_project.be_codebuild_tf_deploy.id
      }
    }
  }
